...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "url_or_path", help="itch.io URL or path to a game jam entries.json file"
    )
    parser.add_argument(
        "--api-key",
        metavar="key",
    ...
    parser.add_argument(
        "--mirror-web", action="store_true", help="try to fetch assets on game sites"
    )
...
def run() -> int:
    ...
    return drive_downloads(
        jobs, download_to, args.mirror_web, settings, keys, parallel=args.parallel
    )
